...
async def get_movies(
    ...
    result = await db.execute(select(MovieModel).order_by(MovieModel.id.desc()).offset(allset).limit(per_page))
    ...
    return MovieListResponseSchema(
        movies = [MovieDetailResponseSchema.model_validate(m, from_attributes=True) for m in movies],
        prev_page=prev_page,
        next_page=next_page,
